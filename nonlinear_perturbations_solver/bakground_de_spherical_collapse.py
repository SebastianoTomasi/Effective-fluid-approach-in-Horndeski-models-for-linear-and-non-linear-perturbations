...
def nonlinear_density_contrast_eq(a,y):
    ...
    fun[1] = -3/(2*a)*(1-effective_eos_a(a))*y[1] + 4/3*y[1]**2/(1+y[0]) \
        +3/(2*a**2)*omega_matter_a(a)*y[0]*(1+y[0])  
    return np.array(fun)
...
def solve_nonlinear_perturbations(delta_coll_star,a_coll):
    ...
    d_delta_m_ini=n*delta_m_ini/a_min#Initial perturbation first derivative w.r.t scale parameter value
    
    init_cond=[delta_m_ini,d_delta_m_ini]# Initial conditions
