# -*- coding: utf-8 -*-
"""
Created on Wed Mar  1 09:43:39 2023
...
def nonlinear_density_contrast_eq(a,y):
    """This function defines the NONLINEAR matter density contrast differential equation for clustering dark energy.
    y[0]=\delta_m
    y[1]=\theta 
    y[2]=\delta_de
    ...
    fun=[0]*3
    fun[0] = -(1+y[0])*y[1]/a
    fun[1] = -(1-3*effective_eos_a(a))/(2*a)*y[1]-y[1]**2/(3*a)-3/(2*a)*(omega_matter_a(a)*y[0]+omega_dark_a(a)*(1+3*c_eff(a))*y[2])
    fun[2] = -3/a*(c_eff(a)-params.de_eos_a(a))*y[2]-(1+params.de_eos_a(a)+(1+c_eff(a))*y[2])*y[1]/a
    
    if fun[2]<-1:#Fisicamente non si può andare sotto -1
        print("delta_de is < -1!!!")
        fun[2]=-1
    return np.array(fun)
...
def linear_density_contrast_eq(a,y):
    ...
    fun=[0]*3
    fun[0] = -y[1]/a
    fun[1] = -(1-3*effective_eos_a(a))/(2*a)*y[1]-3/(2*a)*(omega_matter_a(a)*y[0]+omega_dark_a(a)*(1+3*c_eff(a))*y[2])
    fun[2] = -3/a*(c_eff(a)-params.de_eos_a(a))*y[2]-(1+params.de_eos_a(a))*y[1]/a
    return np.array(fun)
...
def solve_nonlinear_perturbations(delta_coll_star,a_coll):
    ...
    delta_de_ini=n*(1+params.de_eos_a(a_min))*delta_m_ini/(n+3*(c_eff(a_min)-params.de_eos_a(a_min)))#Initial DE perturbation first derivative w.r.t scale parameter value
    ...
    return nonlinear_matter_density_contrast_numerical_a
...
fried_sol=fr_sol.solve()
test=solve(fried_sol,10)
linear_density_contrast_at_collapse=test[0]
virialization_overdensities_stars=test[1]
growth_factirs=test[2]
